...
但是远离平衡点速度收敛速度慢，结合双幂次滑模控制方法，设计双幂次加线性项+$\gamma x_1$,$m_1 > n_1,1/2<p1/q2<1$
$$
s=x_{2}+\alpha_{1} x_{1}^{\left(m_{1} / n_{1}\right)}+\beta_{1} x_{1}^{\left(p_{1} / q_{1}\right)}+\gamma_1 x_1 \tag{4}\label{eq4}
$$
$$
\begin{align}
\dot s&=\dot{x}_2+\alpha_1(m_1/n_2)x^{(m_1/n_1-1)}_1x_2+\beta_1(p_1/q_1)x^{(p_1/q_1-1)}_1x_2+\gamma_1  x_2
\end{align}
$$
​		其中$\beta_1(p_1/q_1)x^{(p_1/q_1-1)}_1x_2$似乎是非奇异项，当$x_1=0,x_2 \neq 0$,但是不会出现这种情况。
$$
\begin{align}
\beta_1(p_1/q_1)x^{(p_1/q_1-1)}_1x_2&=-\beta_1(p_1/q_1)x^{(p_1/q_1-1)}_1(\alpha_{1} x_{1}^{\left(m_{1} / n_{1}\right)}+\beta_{1} x_{1}^{\left(p_{1} / q_{1}\right)}+\gamma_1 x_1)\\
&=-\alpha_1 \beta_1(p_1/q_1)x^{(m_1/n_1+p_1/q_1-1)}_1-\beta^2_1(p_1/q_1)x^{(2p_1/q_1-1)}_1-\beta_1(p_1/q_1)x^{(p_1/q_1)}_1
\end{align}
$$
​	所有不存在奇异现象。
...
​		趋近律设计再利用$\eqref{eq4}$微分方程。不过趋近律不设计奇异性，只需$m_2>n_2,p_2<q_2$
$$
\dot{s}=-\alpha_{2} s^{\frac{m_{2}}{n_{2}}}-\beta_{2} s^{\frac{p_{2}}{q_{2}}}-\gamma s -(\sup(d(t))+\delta)\operatorname{sign}(s)
$$
于是可以设计出相应的$u$,上述方法依次解决传统滑模的问题，但是抖振问题还未证明。
